...
class RknnModel {
 public:
  ...
  std::vector<std::vector<float>> run(const std::vector<float>& input, double* ms = nullptr) {
    ...
    if (ms) *ms = std::chrono::duration<double, std::milli>(
...
static void sample_rgb(const Image& image, float x, float y, float* out) {
...
static std::vector<float> detector_input(const Image& image, Letterbox& box) {
...
static float anchor_scale(int layer) {
  return 0.1484375f + (0.75f - 0.1484375f) * layer / 3.0f;
}
...
static std::vector<Anchor> make_anchors() {
  ...
  for (int layer = 0; layer < 4;) {
    ...
    for (int i = layer; i <= last; ++i) {
      (void)anchor_scale(i);
      float next = i == 3 ? 1.0f : anchor_scale(i + 1);
      (void)next;
      anchors_per_cell += 2;  // aspect 1 + interpolated aspect 1
    }
...
static Detection decode_detection(const std::vector<float>& raw_boxes,
...
static std::vector<float> landmark_input(const Image& image, const Roi& roi) {
...
static std::vector<Point3> project_landmarks(const std::vector<float>& raw,
...
static const int kBlendLandmarks[146] = {
...
static const char* kBlendNames[52] = {
...
static std::vector<float> blend_input(const std::vector<Point3>& points, int w, int h) {
...
static Point3 sub(Point3 a, Point3 b) { return {a.x - b.x, a.y - b.y, a.z - b.z}; }
static Point3 cross(Point3 a, Point3 b) {
  return {a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x};
}
static Point3 normalized(Point3 p) {
  float n = std::sqrt(p.x * p.x + p.y * p.y + p.z * p.z);
  return n > 1e-6f ? Point3{p.x / n, p.y / n, p.z / n} : Point3{0, 0, 0};
}

static std::array<float, 3> head_pose(const std::vector<Point3>& p, int w, int h) {
  ...
  // ponytail: landmark axes are monocular pose; use calibrated canonical-mesh Procrustes for metric AR parity.
  return {yaw, pitch, roll};
}

static void pixel(Image& image, int x, int y, unsigned char r, unsigned char g, unsigned char b) {
  if (x < 0 || y < 0 || x >= image.w || y >= image.h) return;
  size_t i = (y * image.w + x) * 3;
  image.rgb[i] = r; image.rgb[i + 1] = g; image.rgb[i + 2] = b;
}

static void draw_point(Image& image, int x, int y, unsigned char r, unsigned char g, unsigned char b) {
  for (int dy = -1; dy <= 1; ++dy)
    for (int dx = -1; dx <= 1; ++dx) pixel(image, x + dx, y + dy, r, g, b);
}

static void draw_box(Image& image, const Detection& d) {
  int x0 = static_cast<int>(d.x), y0 = static_cast<int>(d.y);
  int x1 = static_cast<int>(d.x + d.w), y1 = static_cast<int>(d.y + d.h);
  for (int x = x0; x <= x1; ++x) {
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    RknnModel detector(model_dir + "/rknn_face_detector.rknn");
    RknnModel landmarker(model_dir + "/rknn_face_landmarks.rknn");
    RknnModel blendshapes(model_dir + "/tf2onnx_face_blendshapes.rknn");

    Letterbox letterbox{};
    double detector_ms = 0, landmark_ms = 0, blend_ms = 0;
    auto detector_outputs = detector.run(detector_input(image, letterbox), &detector_ms);
    Detection detection = decode_detection(detector_outputs[0], detector_outputs[1], letterbox);
    if (detection.score < 0.5f)
      throw std::runtime_error("no face above score 0.5 (best=" + std::to_string(detection.score) + ")");
    ...
    auto landmark_outputs = landmarker.run(landmark_input(image, roi), &landmark_ms);
    float presence = 1.0f / (1.0f + std::exp(-landmark_outputs[1][0]));
    float tongue_out = landmark_outputs[2][0];
    auto landmarks = project_landmarks(landmark_outputs[0], roi, image.w, image.h);
    auto blend_outputs = blendshapes.run(blend_input(landmarks, image.w, image.h), &blend_ms);
    auto pose = head_pose(landmarks, image.w, image.h);

    std::vector<int> order(52);
    std::iota(order.begin(), order.end(), 0);
    std::sort(order.begin(), order.end(), [&](int a, int b) {
      return blend_outputs[0][a] > blend_outputs[0][b];
    });
    ...
    for (int i : order) {
      if (i == 0) continue;
      std::printf("  %-20s %.6f\n", kBlendNames[i], blend_outputs[0][i]);
      if (++shown == 10) break;
    }
    std::printf("npu_ms: detector=%.3f landmarks=%.3f blendshapes=%.3f total=%.3f\n",
                detector_ms, landmark_ms, blend_ms, detector_ms + landmark_ms + blend_ms);

    draw_box(image, detection);
    for (const auto& p : landmarks)
      draw_point(image, static_cast<int>(p.x * image.w), static_cast<int>(p.y * image.h), 30, 255, 90);
