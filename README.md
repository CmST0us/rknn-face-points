...
指定 Bilibili HLS（1600×1280、30fps）经 MPP 解码 + DMABUF + RKNN + `fakesink` 连续验收 45 秒：1290 帧、累计平均 31.1fps、0 drops。HLS 解码按分片突发输出，因此解码到时钟同步显示之间使用非 `leaky` 背压队列；两帧 `leaky` 队列只用于低延迟摄像头输入。
